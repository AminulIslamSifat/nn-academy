  ...
  questions={[
    {
      ...
      prompt: "What are the dimensions of each MNIST image?",
      options: ["28×28 grayscale pixels (784 features when flattened)", "32×32 RGB", "28×28 RGB", "64×64 grayscale"],
      correctIndex: 0,
      explanation: "MNIST images are 28×28 single-channel (grayscale). Flattened to a 784-dimensional vector for MLP input.",
      randomize: true,
    },
    {
      id: "q2",
      ...
      options: ["To keep activations and gradients in a reasonable range, preventing explosion", "To make images look better", "Because softmax requires [0,1] inputs", "To convert to integers"],
      correctIndex: 0,
      explanation: "Raw pixels (0–255) produce large activations → large gradients → numerical instability. Normalizing to [0,1] keeps everything well-behaved.",
      randomize: true,
    },
    {
      id: "q3",
      ...
      options: ["203,530", "200,960", "200,704", "203,786"],
      correctIndex: 0,
      explanation: "W1: 784×256=200,704 + b1: 256 + W2: 256×10=2,560 + b2: 10 = 203,530 total.",
      randomize: false,
    },
    {
      id: "q4",
      type: "multiple-choice",
      prompt: "How many training and test samples does MNIST contain?",
      options: ["60,000 train + 10,000 test", "50,000 train + 10,000 test", "100,000 train + 10,000 test", "60,000 train + 60,000 test"],
      correctIndex: 0,
      explanation: "Standard MNIST split: 60K training images and 10K test images of handwritten digits 0–9.",
      randomize: true,
    },
    {
      id: "q5",
      type: "multiple-choice",
      prompt: "If your MNIST network achieves ~10% accuracy consistently, what's most likely wrong?",
      options: ["Network predicts randomly — check initialization, gradients, and label encoding", "Learning rate is too high", "Need more hidden layers", "MLPs can't solve MNIST"],
      correctIndex: 0,
      explanation: "10% = random chance for 10 classes. The network isn't learning. Check: gradient signs, softmax stability, one-hot encoding, weight init.",
      randomize: true,
    },
    {
      id: "q6",
      type: "multiple-choice",
      prompt: "What activation function is used in the hidden layer of the MNIST MLP?",
      options: ["ReLU", "Sigmoid", "Tanh", "Softmax"],
      correctIndex: 0,
      explanation: "ReLU is the default for hidden layers: fast, no vanishing gradient for positive inputs, sparse activation.",
      randomize: true,
    },
    {
      id: "q7",
      type: "multiple-choice",
      prompt: "What output activation and loss are used for MNIST classification?",
      options: ["Softmax + Categorical Cross-Entropy", "Sigmoid + BCE", "Linear + MSE", "ReLU + MSE"],
      correctIndex: 0,
      explanation: "10-class classification → softmax output (probability distribution) + CCE loss. The natural pairing.",
      randomize: true,
    },
    {
      id: "q8",
      type: "multiple-choice",
      prompt: "Why use He initialization (randn × sqrt(2/fan_in)) for ReLU networks?",
      options: ["Maintains activation variance across layers, accounting for ReLU halving the variance", "Makes weights larger", "It's the only option for ReLU", "Prevents dead neurons entirely"],
      correctIndex: 0,
      explanation: "ReLU zeros out ~half the activations. He init compensates with sqrt(2/n) to keep signal magnitude stable through layers.",
      randomize: true,
    },
    {
      id: "q9",
      type: "multiple-choice",
      prompt: "Why is Adam preferred over vanilla SGD for MNIST training?",
      options: ["Adapts learning rates per-parameter, reaching good accuracy in fewer epochs", "Uses less memory", "Produces better generalization always", "Doesn't need backprop"],
      correctIndex: 0,
      explanation: "Adam's adaptive LR handles different parameter scales automatically. SGD needs careful LR scheduling to match performance.",
      randomize: true,
    },
    {
      id: "q10",
      type: "multiple-choice",
      prompt: "What does shuffling data each epoch prevent?",
      options: ["Biased gradient estimates from ordered batches", "Overfitting", "Memory issues", "Slow computation"],
      correctIndex: 0,
      explanation: "Without shuffling, consecutive batches may be similar (e.g., all digit '0's), giving biased gradients and poor convergence.",
      randomize: true,
    },
    {
      id: "q11",
      type: "multiple-choice",
      prompt: "If loss is NaN during training, what should you check first?",
      options: ["Softmax numerical stability (subtract max) and log clipping", "Increase learning rate", "Add more layers", "Switch optimizer"],
      correctIndex: 0,
      explanation: "NaN usually means exp overflow in softmax or log(0) in cross-entropy. Subtract max before exp, clip before log.",
      randomize: true,
    },
    {
      id: "q12",
      type: "multiple-choice",
      prompt: "If loss doesn't decrease at all, what's the most likely fix?",
      options: ["Adjust learning rate — try 0.001 with Adam", "Add more layers", "Use sigmoid instead of ReLU", "Increase batch size"],
      correctIndex: 0,
      explanation: "LR too high → divergence. LR too low → no progress. Start with Adam lr=0.001 as baseline.",
      randomize: true,
    },
    {
      id: "q13",
      type: "multiple-choice",
      prompt: "What accuracy should a well-trained 2-layer MLP achieve on MNIST?",
      options: ["~97%+", "~90%", "~80%", "~99.5%"],
      correctIndex: 0,
      explanation: "A simple 784→256→10 MLP with proper training reaches ~97%. CNNs push this to ~99%+.",
      randomize: true,
    },
    {
      id: "q14",
      type: "multiple-choice",
      prompt: "Why store Z1 (pre-activations) during forward pass?",
      options: ["Needed for ReLU derivative mask (Z1 > 0) during backward pass", "To compute the loss", "For visualization only", "Not needed — can recompute"],
      correctIndex: 0,
      explanation: "Backward pass needs to know which neurons were active. The mask (Z1 > 0) gates the gradient through ReLU.",
      randomize: true,
    },
    {
      id: "q15",
      type: "multiple-choice",
      prompt: "What shape should the input X have for a batch of 64 MNIST images?",
      options: ["(64, 784)", "(64, 28, 28)", "(784, 64)", "(64,)"],
      correctIndex: 0,
      explanation: "Each image is flattened to 784 features. Batch dimension first: (batch_size, 784).",
      randomize: true,
    },
    {
      id: "q16",
      type: "multiple-choice",
      prompt: "If train accuracy >> test accuracy, what problem do you have?",
      options: ["Overfitting — add regularization or reduce model capacity", "Underfitting", "Data loading bug", "Learning rate too low"],
      correctIndex: 0,
      explanation: "Large train/test gap = overfitting. Solutions: dropout, weight decay, data augmentation, smaller network.",
      randomize: true,
    },
    {
      id: "q17",
      type: "multiple-choice",
      prompt: "What does np.argmax(probs, axis=1) give you?",
      options: ["Predicted class labels for each sample in the batch", "Confidence scores", "Loss values", "Gradient directions"],
      correctIndex: 0,
      explanation: "argmax picks the index of the highest probability for each row → predicted digit class.",
      randomize: true,
    },
    {
      id: "q18",
      type: "multiple-choice",
      prompt: "Why is one-hot encoding needed for CCE loss?",
      options: ["CCE multiplies y_true × log(y_pred) element-wise; one-hot selects only the true class contribution", "It's not needed — integer labels work fine", "For visualization purposes", "To normalize labels"],
      correctIndex: 0,
      explanation: "With one-hot y, only the true class position contributes to −Σ y_c·log(ŷ_c). All other terms are 0×log(...) = 0.",
      randomize: true,
    },
    {
      id: "q19",
      type: "multiple-choice",
      prompt: "What's the main limitation of using an MLP for image classification vs a CNN?",
      options: ["MLPs ignore spatial structure — nearby pixels are treated the same as distant ones", "MLPs are slower", "MLPs can't handle images at all", "MLPs require more data"],
      correctIndex: 0,
      explanation: "Flattening destroys spatial relationships. CNNs use local receptive fields and weight sharing to exploit spatial structure.",
      randomize: true,
    },
    {
      id: "q20",
      type: "multiple-choice",
      prompt: "During training, why compute both train and test accuracy each epoch?",
      options: ["To monitor for overfitting (diverging train/test curves) and underfitting (both low)", "Test accuracy is required for backprop", "Only test accuracy matters", "To speed up training"],
      correctIndex: 0,
      explanation: "Train acc shows learning progress. Test acc shows generalization. Diverging curves = overfitting. Both low = underfitting.",
      randomize: true,
    }
  ]}
/>
